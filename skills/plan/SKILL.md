---
name: plan
description: Plan a complex task by delegating all context-gathering to cheap subagents, then doing the heavy reasoning yourself to produce an actionable Cursor plan with atomic tasks a cheaper model can execute. Use when the user invokes /plan.
...
- `/refine` — critique and tighten an existing plan against the quality bar above.
- `/review` — verify the session's output once the work is done.
